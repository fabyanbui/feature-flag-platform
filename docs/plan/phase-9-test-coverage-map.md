...
1. Continue with release evidence documentation for security, audit append-only
   behavior, demo script, troubleshooting, report, and slides.
2. Run the final full validation sequence after documentation is complete.
...
| Rule order: kill switch/global disable first | Covered | Evaluation engine tests `KILL_SWITCH`, `FLAG_DISABLED`, `GLOBAL_ON`, rule precedence among rule types, and kill-switch precedence over matching targeting rules. | Keep as evidence. |
...
| API validation error shape | Covered | `apps/backend/test/phase-9-api-hardening.e2e-spec.ts`; `apps/backend/src/common/filters/api-exception.filter.spec.ts`; `apps/backend/test/integration/phase-3-foundation.integration-spec.ts`; DTO specs under `apps/backend/src/common/dto/` | Keep as release-readiness evidence. |
| Pagination response shape | Covered | `apps/backend/test/phase-9-api-hardening.e2e-spec.ts`; `apps/backend/src/common/dto/page-response.dto.spec.ts`; `apps/backend/src/projects/projects.service.spec.ts`; `apps/backend/src/feature-flags/feature-flags.service.spec.ts`; `apps/backend/src/audit-logs/audit-logs.service.spec.ts`; `apps/backend/test/phase-5-management.e2e-spec.ts` | Keep as evidence. |
| Conflict behavior | Covered | `apps/backend/test/phase-9-api-hardening.e2e-spec.ts`; `apps/backend/test/phase-5-management.e2e-spec.ts`; `apps/backend/src/projects/projects.service.spec.ts`; `apps/backend/src/feature-flags/feature-flags.service.spec.ts`; `apps/backend/src/flag-rules/flag-rules.service.spec.ts` | Keep as evidence. |
| Missing actor rejected for mutations | Covered | `apps/backend/test/phase-9-api-hardening.e2e-spec.ts`; `apps/backend/src/common/guards/actor-required.guard.spec.ts`; `apps/backend/test/phase-5-management.e2e-spec.ts`; management service specs | Keep as evidence. |
| Unsupported sort/filter rejected safely | Covered | `apps/backend/test/phase-9-api-hardening.e2e-spec.ts`; project, feature flag, flag rules, sample users, and audit log service specs | Keep as evidence. |
